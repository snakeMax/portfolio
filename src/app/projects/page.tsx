...
const LOCAL_PROJECTS: Project[] = [
  // Eksempelprosjekt – legg til flere lokalt hvis ønskelig
  // {
  //   title: "Auth Gateway",
...
export default async function Page() {
  ...
  if (REMOTE) {
    try {
      ...
      // Ignorer feil – fallback til LOCAL_PROJECTS
    }
  }
    ...
    <div className='flex flex-col items-center min-h-screen'>
      <div className='bg-black/80 w-full max-w-[800px] rounded-xl shadow-[0_0_40px_rgba(0,255,0,0.6)] border border-green-500/20 p-8 my-8'>
        <div className='flex flex-col items-center'>
          <h1 className='text-4xl font-bold my-4 text-white'>Projects</h1>
            ...
            <ul className='grid w-full gap-4'>
              {projects.map((p) => (
                ...
                >
                  ...
                  {p.summary ? (
                    <p className='mt-2 text-white/90'>{p.summary}</p>
                  ) : null}
...
// Husk å sette NEXT_PUBLIC_PROJECTS_JSON i .env.local når det er klart!
// Opprett en .env.local i rotmappa med f.eks:
// NEXT_PUBLIC_PROJECTS_JSON=https://your-domain.example.com/projects.json
